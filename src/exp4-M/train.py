# train.py  —  Exp-4 (RKD-Distance only, single-phase, teacher único)
# - TODO sale del YAML (phase4M.yaml u otro con --config).
# - Backbones: mobilenetv3, efficientnetv2_m (definidos en models.py).
# - Teacher único: ResNet-50 (definido en YAML).
# - Artefactos SIEMPRE en: modelFER/outputs/phase3/<backbone>/exp4/{checkpoints,csv,exports,calibration}
# - ZIPs fuera de outputs/: exports/<backbone>/exp4/exports/...
# - Incluye: tqdm, AMP, resume, early-stopping, sampler balanceado, monitor recursos, CSV por epoch.
# - ZIP por mejora (best_1.zip, best_2.zip, ...) con checkpoint ligero + CSV; y ZIP final ligero.
# - CORRECCIÓN: RAF-DB val ya NO usa load_rafdb("val"); el split 90/10 se hace EN MEMORIA → no crea _splits en disco.
...
# Distillation Loss (RKD-Distance + CE + KL opcional)
# En Exp-4 usamos: beta=0.0 (sin KL), gamma>0 (distance), use_angle=False

class DistillationLoss(nn.Module):
    def __init__(self, kd_cfg, ce_weight=None, label_smoothing=0.0):
        super().__init__()
        self.alpha = kd_cfg.get("alpha", 0.55)      # CE
        self.beta = kd_cfg.get("beta", 0.0)         # KL (Response KD)
        self.gamma = kd_cfg.get("gamma", 0.45)      # RKD distance
        self.temperature = kd_cfg.get("temperature", 4.0)
        self.use_distance = kd_cfg.get("use_distance", True)
        self.use_angle = kd_cfg.get("use_angle", False)  # en Exp-4: False
    ...
    def rkd_angle_loss(self, f_s, f_t):
        # opcional (no usado en Exp-4)
        with torch.no_grad():
    ...
    def forward(self, student_logits, teacher_logits, targets,
        ...
        # KL (Response KD) — apagada en Exp-4 (beta=0)
        kd_loss = 0.0
        if self.beta > 0.0:
            kd_loss = F.kl_div(
        ...
        # Relation KD (distance [+ angle opcional])
        rkd_loss = 0.0
        if (student_features is not None) and (teacher_features is not None):
            if self.use_distance:
                rkd_loss = rkd_loss + self.rkd_distance_loss(student_features, teacher_features)
            if self.use_angle:
                rkd_loss = rkd_loss + self.rkd_angle_loss(student_features, teacher_features)
...
def next_zip_path(zip_dir, backbone, idx):
    """Construye un nombre único por mejora: exports/<backbone>/exp4/exports/best_<backbone>_<idx>.zip"""
...
def train_one_epoch(model, teacher, loader, optimizer, scheduler, device, criterion,
                    epoch, kd_warmup_epochs=3, grad_clip_norm=None,
...
def main():
    ...
    parser.add_argument("--config", type=str, default="/kaggle/working/modelFER/configs/phase4M.yaml", help="Ruta al YAML de configuración")
    ...
    base_out_dir = os.path.join(out_root, backbone, "exp4")            # …/modelFER/outputs/phase3/<backbone>/exp4

    # Subcarpetas estándar (todas bajo modelFER/outputs/phase3/...)
    ...
    # ZIPs EXTERNOS (fuera de outputs/), tal como veníamos:
    external_zip_dir = to_project_path(os.path.join("exports", backbone, "exp4", "exports"))
    ensure_dir(external_zip_dir)
    zip_path = os.path.join(external_zip_dir, f"exp4_results_{backbone}.zip")
    ...
    # DataLoaders para los subsets de RAF
    ...
    # Loader provisional (puede ser reemplazado por sampler balanceado más abajo)
    train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True,
                              num_workers=num_workers, pin_memory=True, drop_last=True)
    val_loader   = DataLoader(val_dataset, batch_size=batch_size, shuffle=False,
                              num_workers=num_workers, pin_memory=True)

    # --- Class weights (opcional) SOBRE LA MEZCLA (AffectNet + RAF train_subset)
    ce_weight = None
    try:
    ...
    # --- (OPCIONAL) WeightedRandomSampler para balanceo por clase en train ---
    use_weighted_sampler = True  # ponlo en False si prefieres solo shuffle=True
    ...
    if optimizer_name == "adamw":
        ...
        optimizer = optim.SGD(student.parameters(), lr=lr, momentum=0.9, weight_decay=wd, nesterov=True)
    else:
    ...
    kd_warmup_epochs = int(cfg["train"].get("kd_warmup_epochs", 3))
    ...
    for epoch in range(start_epoch, epochs):
        ...
        train_loss, train_acc = train_one_epoch(
            student, teacher, train_loader, optimizer, scheduler, device, criterion,
        ...
        # --- Calibración: guardar SIEMPRE en .../outputs/phase3/<backbone>/exp4/calibration
        try:
            ece_val = None
